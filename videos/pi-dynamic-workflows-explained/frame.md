...
## Fonts — local faces (staged by the orchestrator; preset fonts kept)

The preset's own faces ship as local WOFF2 files in `assets/fonts/`. Name these
families only; each is declared here so Studio, snapshots, and the render
resolve offline with no Google Fonts fetch:
...
@font-face{font-family:"EB Garamond";font-weight:700;font-style:normal;font-display:block;src:url("assets/fonts/EBGaramond-700.woff2") format("woff2");}
